...
def fetch_interview_context(room_name: str) -> Optional[Tuple[Dict, Dict, Dict, int, str, Dict]]:
    ...
    - db_ids: Internal UUIDs for logging.
    """
    with get_db() as db:
        try:
            try:
            ...
            job = schedule.job
            interview_round = schedule.interview_round # This is InterviewRounds model
            ...
            # --- 3. Fetch Specific Agent Configuration ---
            round_data = None
            db_round_list_id = None # This is the ID of the round *definition*

            if interview_round and interview_round.round_details:
                # This is the 'round_list.id' we need to find the config
                db_round_list_id = interview_round.round_details.id
                
                # Query for the specific config for this job and this round
                config_query = (
                ...
                if agent_config:
                    # Found it! Use the rich config from the DB.
                    ...
                    # No specific config found. Fall back to default round info.
                    logger.warning(f"No AgentRoundConfig found for job {job.id}, round {db_round_list_id}. Using default round description.")
                    round_data = {
                        "round_name": interview_round.round_details.round_name,
                        "round_description": interview_round.round_details.round_description
                        # This object will NOT have persona, skills, etc.
                    }
        ...
        try:
            ...
            if 'transcripts' in msg or 'undefinedtable' in msg or 'relation "transcripts"' in msg:
                logger.error(
                    ...
                    "Apply the migration at `RMS-B/scripts/migrations/004_create_transcripts.sql` or create the table in your DB."
                )
                return False
